...
/* The standalone pin-name patch (dist\pin-names-patch) is a game_engine.dll of its
   own, built from the same source with TC_PIN_PATCH_ONLY.  It is not a loader, but
   it is a known occupant of that file, so the installer accepts it and replaces it
   instead of refusing with "another loader is installed".  It carries an export of
   its own; the export's name is what is looked for, in the file - nothing of the
   patch is ever loaded or run here. */
static bool standalonePatch(const tc::fs::path& engine){
 return tc::read(engine).find("tc_pin_patch_marker")!=std::string::npos;
}
static void install(const tc::fs::path& root,bool remove){
 ...
 if(tc::hash(tc::read(exe))!=TC_EXE_SHA)throw std::runtime_error("Unsupported Turing Complete.exe build. Expected compatibility profile " TC_COMPAT_PROFILE_ID ".");
auto bytes=payload();auto deployed=tc::hash(bytes);auto actual=tc::hash(tc::read(engine));
/* What is in game_engine.dll right now: the original engine, an older loader, this
   release's loader, or the standalone pin-name patch. */
const bool patch=standalonePatch(engine);
 if(remove){
 if(actual==TC_ENGINE_SHA&&!tc::fs::exists(backup))return;
 /* The pin-name patch counts as "ours": uninstalling over it puts the original
    engine back, which is what a player swapping the two would expect. */
 if(actual!=deployed&&!patch)throw std::runtime_error("Engine changed since installation. Refusing to overwrite it.");
...
if(actual!=TC_ENGINE_SHA&&actual!=deployed&&actual!=TC_PREVIOUS_LOADER_SHA&&actual!=TC_LOADER_020_SHA&&!patch)throw std::runtime_error("Engine is unsupported or another loader is installed.");
...
int WINAPI wWinMain(HINSTANCE,HINSTANCE,PWSTR,int){bool silent=false;try{int argc{};LPWSTR* argv=CommandLineToArgvW(GetCommandLineW(),&argc);bool remove=false;tc::fs::path root;
 if(argc==3&&(std::wstring(argv[1])==L"--install"||std::wstring(argv[1])==L"--uninstall")){silent=true;remove=std::wstring(argv[1])==L"--uninstall";root=tc::fs::absolute(argv[2]).lexically_normal();}
  ...
  if(!GetOpenFileNameW(&o)){LocalFree(argv);return 0;}root=tc::fs::path(file).parent_path();int choice=MessageBoxW(nullptr,L"安装 TC Mod Loader？\n\n是：安装 / 升级 / 修复\n否：卸载并恢复 Mod 修改的文件\n取消：退出\n\n安装后正常启动游戏，主菜单右上角会出现 Mods。",L"TC Mod Loader " TC_MODLOADER_VERSION_WTEXT,MB_YESNOCANCEL|MB_ICONQUESTION);if(choice==IDCANCEL){LocalFree(argv);return 0;}remove=choice==IDNO;}
