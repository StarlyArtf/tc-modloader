...
元件注册目前只是原型表操作，不等于完整新元件行为。2026-09-16 的静态研究已确认文件解析型 `add_custom_prototype` 的隐藏返回参数及注册／编译链路，并修正自定义 ID 为 64 位；其运行时所有权、错误处理和完整放置／仿真仍需验证。证据与复现方法见 [元件链路研究](research/COMPONENT-PIPELINE.md)。
