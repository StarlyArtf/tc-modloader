...
元件接口现提供实验性的电路文件导入、目录更新和原型快照释放（`tc_component_model.h`），已在隔离游戏实例验证；也已修正 64 位自定义 ID 和字符串长度。完整菜单、放置、连线和新元件仿真仍未验收，不能等同于任意新逻辑元件支持。证据与复现方法见 [元件链路研究](research/COMPONENT-PIPELINE.md)。
