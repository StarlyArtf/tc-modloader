...
extern "C" TC_MOD_EXPORT int tc_mod_load(const TCHost* h, TCPlugin* plugin) {
    ...
    /* Development fixture: the byte adder with deliberately long pin names, so
       the game's own component drawer draws the case a player reports - names
       wider than the pin slot, overlapping their neighbours.  Behaviour and pin
       shapes are the example's, so build/nl_adder8board.data still drives it. */
    component.id = 0x414444385F303031ULL;
    component.name = "Byte Adder (long pins)";
    component.inputs = {{"Memory address", 1}, {"read data bus", 8}, {"alu result", 8}};
    component.outputs = {{"Store enable", 8}, {"Write data out", 1}};
    component.gates = 1;
    component.delay = 1;
