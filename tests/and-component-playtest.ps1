...
$taskTest = Join-Path $taskRepo ('build\and-component-playtest-' + [guid]::NewGuid().ToString('N'))
$taskRoot = Join-Path $taskTest 'game'
$taskProfile = Join-Path $taskTest 'home'
$taskData = Join-Path $taskRoot 'tc-modloader-data\plugin-data\test.and-component'
...
Copy-Item -LiteralPath (Join-Path $taskRepo 'build\and-component-probe.dll') -Destination (Join-Path $taskPackage 'native\probe.dll')
'{"format":2,"id":"test.and-component","name":"Development AND component probe","version":"0.0.1","native":{"api":1,"entry":"native/probe.dll"}}' | Set-Content -LiteralPath (Join-Path $taskPackage 'mod.json') -Encoding utf8
& (Join-Path $taskRepo 'tools\Pack-Mod.ps1') -Source $taskPackage -Output (Join-Path $taskRoot 'mods\probe.mod') | Out-Null
& (Join-Path $taskRepo 'dist\tcmod-cli.exe') $taskRoot apply test.and-component
if($LASTEXITCODE){throw 'AND probe package apply failed'}
...
try {
  ...
  if(!(Test-Path -LiteralPath $taskResult)) {
    throw "AND probe did not finish; inspect $taskRoot\tc-modloader-data\loader.log"
  }
  $taskText = [IO.File]::ReadAllText($taskResult)
  $taskText
  if(!$taskText.StartsWith('PASS ')) { throw "AND probe failed: $taskText" }
} finally {
