...
try {
 ...
 Compress-Archive -Path src,vendor,sdk,tools,tests,examples,build.ps1,package.ps1,README.md,MOD-FORMAT.md,SDK-GUIDE.md,VALIDATION.md,LICENSE -DestinationPath dist\TCModLoader-0.4.0-source.zip -Force
 Copy-Item dist\TCModLoader-Setup.exe,dist\example.menu-demo.mod,dist\example.cycle-guard.mod,dist\example.circuit-and.mod,dist\dev.cost-watch.mod,dist\TCModLoader-0.4.0-source.zip,README.md,MOD-FORMAT.md,SDK-GUIDE.md,VALIDATION.md,LICENSE $taskStage
 ...
 Compress-Archive -Path (Join-Path $taskStage '*') -DestinationPath dist\TCModLoader-0.4.0-win64.zip -Force
 Get-Item dist\TCModLoader-0.4.0-win64.zip,dist\TCModLoader-0.4.0-source.zip | Select-Object Name,Length
